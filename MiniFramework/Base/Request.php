...
class Request
{
    ...
    public function getBaseUrl()
    {
        if ($this->_baseUrl === null) {
            ...
            if (defined('ABSPATH') && function_exists('is_admin') && is_admin() && function_exists('admin_url')) {
                // For admin pages, the base URL for links generated by the framework
                // might be considered the admin.php page itself.
                // Links to controllers/actions will be built upon this using mf_plugin_url().
                $this->_baseUrl = admin_url('admin.php');
            } else {
                // Original logic to determine base URL
                // setBaseUrl() populates _baseUrl based on PHP_SELF
                // Let's ensure the original logic from setBaseUrl is preserved here
                // The original setBaseUrl was:
                // $urlArray = explode('/', $_SERVER['PHP_SELF']);
                // unset($urlArray[count($urlArray) - 1]);
                // $this->_baseUrl = implode('/', $urlArray);
                // This logic is simple and might be what's intended for non-WP CLI or web access.

                // The example snippet provided a more complex logic:
                // $scriptName = $this->getScriptName(); // This method doesn't exist. Use $_SERVER['SCRIPT_NAME']
                // $requestUri = $this->getRequestUri(); // This method exists.
                // if (0 === strpos($requestUri, $scriptName)) {
                //     $this->_baseUrl = $scriptName;
                // } else {
                //     $baseUrl = rtrim(dirname($scriptName), '/');
                //     $this->_baseUrl = ($baseUrl == DIRECTORY_SEPARATOR || $baseUrl == '.') ? '' : $baseUrl;
                // }
                // Let's use the original setBaseUrl logic for the else case for now,
                // as it's simpler and was the existing behavior.
                // If more complex behavior for non-WP is needed, the example can be adapted.
